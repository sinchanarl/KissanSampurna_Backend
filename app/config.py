...
class Settings(BaseSettings):
    ...
    engine_str: str = f"{os.getenv('DB_DIALECT')}+{os.getenv('DB_DRIVER')}://{os.getenv('DB_USER')}:{os.getenv('DB_PASSWORD')}@{os.getenv('DB_HOST')}:{os.getenv('DB_PORT')}/{os.getenv('DB_NAME')}" \
...
def get_engine(schema: str):
    ...
    try:
        ...
        # Missing DB driver (e.g., aiosqlite) in development environment.
        # Return None as a fallback so the app can start; DB operations
        # will need the real engine in production.
        print("[config] warning: could not create async engine:", exc)
        return None
